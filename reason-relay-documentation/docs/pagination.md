...
let make = (~query as queryRef) => {
  let ReasonRelay.{data, hasNext, isLoadingNext, loadNext} =
    ...
    // NOTE: Functions for collecting the connection nodes like below will soon be autogenerated
    let tickets =
...
3. We gather up all the connection nodes we have. _NOTE:_ This workflow for this will shortly be improved.
4. We render each node using the `<SingleTicket />` component, who's data demands we spread on the `node` of our refetchable fragment. We unwrap `SingleTicket`'s fragment using `Fragment.unwrapFragment_node` before passing it to `<SingleTicket />`.
