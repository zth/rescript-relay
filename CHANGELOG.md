...
- A `RescriptRelay.getConnectionID(parentDataId, connectionKey, 'filters)` is also exposed for the same reason as above, but should be avoided in any situation where the generated function from the fragment can be used.
...
_[Here's a commit showing a project being upgraded to this version](https://github.com/zth/rescript-relay/commit/5aea01a511f9c084a23a8db09c30bbecca59ca32)_

This is the final release before RescriptRelay will rename to RescriptRelay! Do this to upgrade:

1. We now depend on `@rescript/react` instead of `reason-react`. _Remove_ `reason-react` and add `@rescript/react@0.10.0`.
2. The compiler now outputs `res` instead of `re`. For this to work smoothly, you should _delete your entire `__generated__` directory_ and re-run the RescriptRelay compiler.
...
_[Here's a commit showing a project being upgraded to this version](https://github.com/zth/rescript-relay/commit/bf0599b7b5078ff73a4b91cfb30a6c742037852e)_
...
- _BREAKING CHANGE_ Lists of connection ids passed into the store updater directives (@appendEdge, @appendNode, etc) are now properly typed a `array<RescriptRelay.dataId>` rather than `array<string>`.
...
_[Here's a commit showing a project being upgraded to this version](https://github.com/zth/rescript-relay/commit/4c8fb3d36dfa84267cb28ffb081a30a9f55c6554)_

> Make sure you read the breaking changes below!

This release marks the start of RescriptRelay's journey towards two things:
...
- RescriptRelay will be renamed, probably to ReScriptRelay.
...
[Check out the sneak peak release](https://rescript-relay-documentation.zth.now.sh/docs/vscode-extension) of the dedicated VSCode extension.
...
- Support for using RescriptRelay with ReScript ([@sorenhoyer](https://github.com/sorenhoyer))
- Relay `10.1.0`

[Check out this](https://github.com/zth/rescript-relay/commit/f95bbf9d9eb35297e9b48dec2c098e48c391f8d6) for an example diff of upgrading a project.
...
- If you get an error with `bs-platform` > `8.3.0` for code looking like this `let RescriptRelay.{data, hasNext} = ...`, just remove `RescriptRelay.` from that code and the error should be gone. Later versions of `bs-platform` now properly infer record destruction, which prior versions did not do.

## Breaking changes

- _BREAKING CHANGE_ Replace unsetValue with setValueToUndefined and setValueToNull [#105](https://github.com/zth/rescript-relay/pull/105) ([@tsnobip](https://github.com/tsnobip))
- _BREAKING CHANGE_ Replace `React.useTransition(~config={timeoutMs: 5000}, ())` with `ReactExperimental.unstable_useTransition()` [#121](https://github.com/zth/rescript-relay/pull/121) ([@sorenhoyer](https://github.com/sorenhoyer))
- _BREAKING CHANGE_ Replace `React.useDeferredValue` with `ReactExperimental.unstable_useDeferredValue` [#121](https://github.com/zth/rescript-relay/pull/121) ([@sorenhoyer](https://github.com/sorenhoyer))
- _BREAKING CHANGE_ Replace `ReactDOMRe.Experimental.createRoot` with `ReactDOMExperimental.unstable_createRoot` [#121](https://github.com/zth/rescript-relay/pull/121) ([@sorenhoyer](https://github.com/sorenhoyer))
...
- Bind `readInlineData` for fragments annotated with `@inline` [#117](https://github.com/zth/rescript-relay/pull/117) ([@zth](https://github.com/zth))
- Clean bindings, renamed internal raw types and functions with names ending with `Raw` [#105](https://github.com/zth/rescript-relay/pull/105) ([@tsnobip](https://github.com/tsnobip))

## Fixes & misc

- Relay upgraded to `10.1.0` ([@zth](https://github.com/zth))
- Generate a `commitLocalPayload` for any query annotated with `@raw_response_type`, to allow comitting local only payloads in a type safe way. [#118](https://github.com/zth/rescript-relay/pull/118) ([@zth](https://github.com/zth))
- Use abstract records instead of Js.t objects for a more robust type-check and to avoid undefined fields [#105](https://github.com/zth/rescript-relay/pull/105) ([@tsnobip](https://github.com/tsnobip))
- Add support for parsing ReScript (.res) files [#115](https://github.com/zth/rescript-relay/pull/115) ([@sorenhoyer](https://github.com/sorenhoyer))
...
[Check out this](https://github.com/zth/rescript-relay/commit/7880e10aad73c8f302b5cb2eeef424c6d8fc8368) for an example diff of upgrading a project.
...
[Check out this](https://github.com/zth/rescript-relay/commit/8ed636db21a2de5648cc0ff85d2827997a195654) for an example diff of upgrading a project.
...
- Automatic conversion of custom scalars! Heavily inspired by the upcoming `graphql_ppx` release which also has this, it's now possible to have RescriptRelay automatically convert your custom scalars at runtime. More info in the docs for custom scalars.
...
- Fragments can now be on unions, which previously wasn't supported in RescriptRelay (but has always been valid in Relay itself). Thanks to [Arnar Kári Ágústsson](https://github.com/Arnarkari93) for finding this!
...
- Autogenerate helper function for turning a @connection into an array of nodes. This replaces the functionality of the previous `RescriptRelayUtils.collectConnectionNodes` that's no longer usable after moving from `Js.t` to records.
...
- _BREAKING CHANGE_ Add `'response` as second parameter to mutation/subscription `updater` functions. This was simply not bound before in RescriptRelay, but Relay has always provided it.
...
- _SUPER BREAKING CHANGE_ All emitted types are now _records_ instead of _Js.t_, and `Js.Nullable.t`/enums/unions are now autoconverted with _no manual intervention needed_. This should _greatly_ simplify working with RescriptRelay.
- _BREAKING CHANGE_ Remove `collectConnectionNodes` from `RescriptRelayUtils`, since it operated on `Js.t` and nothing is typed as `Js.t` anymore.
...
- RescriptRelay now depends on `react@experimental` and `react-dom@experimental`.
