...
let resolveNestedRecord = (~rootRecord: option<ReasonRelay.RecordProxy.t>, ~path: list<string>) => {
  ...
  | _ =>
    for i in 0 to pathLength - 1 {
      ...
      | (Some(record), Some(currentPath)) =>
        currentRecord := record->ReasonRelay.RecordProxy.getLinkedRecord(~name=currentPath, ())
...
let resolveNestedRecordFromRoot = (~store, ~path: list<string>) =>
  ...
  | list{} => None
  | list{rootRecordPath} =>
    switch store->ReasonRelay.RecordSourceSelectorProxy.getRootField(~fieldName=rootRecordPath) {
    | Some(rootRecord) => Some(rootRecord)
    | None => None
    }
  | list{rootRecordPath, ...restPath} =>
    resolveNestedRecord(
      ~rootRecord=store->ReasonRelay.RecordSourceSelectorProxy.getRootField(
