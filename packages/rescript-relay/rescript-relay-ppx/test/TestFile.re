...
describe("extractFragmentConnectionInfo", ({test, _}) => {
  test("it should extract the connection key", ({expect}) => {
    let connectionInfo =
      Util.extractFragmentConnectionInfo(
        ~loc=makeMockLocObj(),
        Util.extractGraphQLOperation(
          ~loc=makeMockLocObj(),
          {|
        fragment SomeFragment_someProp on SomeEntity {
          ...
          someConnectionField(
            first: $first,
            after: $after,
          ) @connection(key: "SomeFragment_someProp_someConnectionField") {
            edges {
              node {
                id
              }
    ...
    expect.option(connectionInfo).toBe(
      Some({
        key: "SomeFragment_someProp_someConnectionField",
  ...
  test(
    "it should extract all applicable filters if no explicit filter config is set",
    ({expect}) => {
    let connectionInfo =
      Util.extractFragmentConnectionInfo(
        ~loc=makeMockLocObj(),
    ...
    expect.option(connectionInfo).toBe(
      Some({
        key: "SomeFragment_someProp_someConnectionField",
        applicableFilterKeys: ["someProp", "anotherProp"],
      }),
    );
  });

  test("it should take turning filters off into account", ({expect}) => {
    let connectionInfo =
      Util.extractFragmentConnectionInfo(
        ~loc=makeMockLocObj(),
        Util.extractGraphQLOperation(
        ...
        fragment SomeFragment_someProp on SomeEntity {
          ...
          someConnectionField(
            ...
            someProp: true,
            anotherProp: $anotherProp
          ) @connection(key: "SomeFragment_someProp_someConnectionField", filters: []) {
            edges {
              node {
                id
              }
  ...
  test("it should take any filters config into account", ({expect}) => {
    let connectionInfo =
      Util.extractFragmentConnectionInfo(
        ~loc=makeMockLocObj(),
        Util.extractGraphQLOperation(
        ...
        fragment SomeFragment_someProp on SomeEntity {
          ...
          someConnectionField(
            ...
            someProp: true,
            anotherProp: $anotherProp
          ) @connection(key: "SomeFragment_someProp_someConnectionField", filters: ["someProp"]) {
            edges {
              node {
                id
              }
