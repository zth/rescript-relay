...
function buildConversionModules(destination) {
  ...
  for (const name of ['utils','prepareConversion']) {
    ...
    if (!exports.test(source)) throw new Error('Unsupported conversion exports in '+name);
    source = source.replace(exports,'export {$1};\n');
    if (name === 'utils') {
      const dependency = 'const { prepareConversion } = require("./prepareConversion");';
      if (!source.includes(dependency)) throw new Error('Missing prepared-converter import');
      source = source.replace(dependency,'import { prepareConversion } from "./prepareConversion.mjs";');
    }
    fs.writeFileSync(path.join(destination,name+'.mjs'),source);
  }
}
if (require.main === module) {
  if (!process.argv[2]) throw new Error('Usage: build-conversion-modules.js <output-directory>');
