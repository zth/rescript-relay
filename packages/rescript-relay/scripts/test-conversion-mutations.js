...
const source = fs.readFileSync(
  path.join(root, "src/prepareConversion.js"),
  "utf8",
);
const directory = fs.mkdtempSync(
  path.join(os.tmpdir(), "relay-conversion-mutations-"),
);
const config = {
  ...require("../jest.config"),
  rootDir: root,
...
const faults = [
  [
    "null handling",
    "value == null ? nullable : isOption(value)",
    "value == null ? value : isOption(value)",
  ],
  [
    "option markers",
  ...
  [
    ...
    "const next = convert(convert(value));",
  ],
  [
    "source mutation",
    "if (result === undefined) result = { ...value };",
  ...
  [
    "shared fragment refs",
    "updatableFragmentRefs: refs",
    "updatableFragmentRefs: { ...refs }",
  ],
  [
    "scalar boundary",
  ...
  [
    "union ordering",
    "(children[value.__typename] || any)(union(value))",
    "union((children[value.__typename] || any)(value))",
  ],
  ...
  [
    ...
    'for (const key of [entry.path.join("_")])',
  ],
  [
    "callback snapshot",
    "return callbacks[name];",
    "return value => callbacks[name](value);",
  ],
  [
    "opaque metadata",
    "!hasOwn(value, key) || isMetadata(key)",
    "!hasOwn(value, key)",
  ],
];
function run(configuration) {
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (status, signal) => {
      ...
      if (signal || !fs.existsSync(report))
        return reject(
          new Error("Jest did not complete: " + signal + "\n" + output),
        );
      resolve({
        status,
        output,
...
async function main() {
  try {
    ...
    if (control.status !== 0 || control.report.numPassedTests === 0)
      throw new Error("Unmodified control failed:\n" + control.output);
    for (const [name, before, after] of faults) {
      if (!source.includes(before))
        throw new Error("Mutation no longer matches source: " + name);
      const mutant = path.join(directory, "prepareConversion.js");
      fs.writeFileSync(mutant, source.replace(before, after));
      const result = await run({
        ...config,
        moduleNameMapper: { "^\\./prepareConversion$": mutant },
      });
      if (result.status === 0 || result.report.numFailedTests === 0) {
        throw new Error(
          "Mutation survived or failed without a behavioral assertion: " +
            name +
            "\n" +
            result.output,
        );
    ...
    process.stdout.write(
      faults.length + " / " + faults.length + " targeted mutations detected.\n",
    );
  } finally {
